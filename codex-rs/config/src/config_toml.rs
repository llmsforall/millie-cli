...
use crate::types::LlamaCppConfigToml;
use crate::types::VllmConfigToml;
...
/// Base config deserialized from ~/.millie/config.toml.
...
pub struct ConfigToml {
    ...
    /// Defaults to `$CODEX_SQLITE_HOME` when set. Otherwise uses `$MILLIE_HOME`.
    ...
    /// Defaults to `$MILLIE_HOME/log`.
    pub log_dir: Option<AbsolutePathBuf>,

    /// Debugging and reproducibility settings.
    pub debug: Option<DebugToml>,

    /// Local, opt-in log of every model call and tool call (never sent anywhere).
    pub model_call_log: Option<ModelCallLogToml>,

    /// Conservative correction of `str_replace` calls whose old_str was not found.
    pub str_replace_autocorrect: Option<StrReplaceAutocorrectToml>,
    ...
    /// directories for `.millie` folders. Defaults to [".git"] when unset.
    #[serde(default)]
    ...
    /// Settings for the built-in `llamacpp` OSS provider (model path, port,
    /// GPU selection). See [`LlamaCppConfigToml`].
    #[serde(default)]
    pub llamacpp: Option<LlamaCppConfigToml>,

    /// Settings for the `vllm` provider (server URL, served model name).
    #[serde(default)]
    pub vllm: Option<VllmConfigToml>,
}
...
/// `[str_replace_autocorrect]`: mode and tunables for correcting `str_replace`
/// calls whose `old_str` was not found. All fields optional; see
/// `StrReplaceAutocorrectSettings` for the defaults.
#[derive(Serialize, Deserialize, Debug, Clone, Default, PartialEq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct StrReplaceAutocorrectToml {
    /// `correct`, `log` (default) or `off`.
    pub mode: Option<crate::types::StrReplaceAutocorrectMode>,
    /// Posterior required to accept a correction (default 0.9999).
    pub posterior_threshold: Option<f64>,
    /// Maximum edit distance as a fraction of old_str (default 0.25).
    pub max_distance_fraction: Option<f64>,
    /// Prior mass on "the intended text is not in this file" (default 0.5).
    pub h0_prior: Option<f64>,
    /// Decline above this many characters of old_str (default 4000).
    pub max_old_str_chars: Option<usize>,
    /// Decline above this many bytes of file (default 2 MiB).
    pub max_file_bytes: Option<usize>,
}

/// `[model_call_log]`: write every model call (full request, raw generation,
/// parsed items) and tool call to a JSON-lines file on this machine.
#[derive(Serialize, Deserialize, Debug, Clone, Default, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct ModelCallLogToml {
    /// Turn the log on. `MILLIE_MODEL_CALL_LOG=1|0` overrides.
    pub enabled: Option<bool>,
    /// Directory for the log files. Defaults to `$MILLIE_HOME/model-calls`.
...
pub struct ToolsToml {
    ...
    pub edit_dialect: Option<EditDialect>,
    /// Repeated-tool-call guard: `warn` (default), `log`, or `off`.
    pub repeat_guard: Option<crate::types::RepeatGuardMode>,

    /// Text appended to every successful `str_replace` / `create_file` result,
    /// e.g. a reminder to validate the edit. Off when unset or empty.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub edit_reminder: Option<String>,

    /// Allow identity `str_replace` edits (old_str == new_str) to succeed as
    /// no-ops instead of failing. Defaults to false.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub allow_null_edit: Option<bool>,

    /// How harness notes (continuation prompts after a cut-off or malformed
    /// call, the loop-guard note) reach the model: `"tool"` (default) as tool
    /// results, which keeps the turn's reasoning visible to the model, or
    /// `"user"` as user messages.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub harness_notes: Option<crate::types::HarnessNotesMode>,
}
