...
impl ConfigLayerEntry {
    ...
    // Get the `.millie/` folder associated with this config layer, if any.
    ...
    /// Returns the `.millie/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$MILLIE_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$MILLIE_HOME/config.toml` layer because the active layer is the
