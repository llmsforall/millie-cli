...
#[test]
fn sampling_keys_reject_typos_without_strict_mode() {
    let error = toml::from_str::<crate::config_toml::ConfigToml>("[llamacpp]\ntemperture = 1.0\n")
        .unwrap_err();
    assert!(error.to_string().contains("temperture"));
    let config: crate::config_toml::ConfigToml =
        toml::from_str("[llamacpp]\ntemperature = 1.0\nmin_p = 0.0\n").unwrap();
    assert_eq!(config.llamacpp.unwrap().min_p, Some(0.0));
}
