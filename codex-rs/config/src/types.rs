//! Types used to define loaded and effective Millie configuration values.
...
/// Determine where Millie should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in MILLIE_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in MILLIE_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Millie should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    /// `Keyring` when available; otherwise, `File`.
    /// Credentials stored in the keyring will only be readable by Millie unless the user explicitly grants access via OS-level keyring access.
    #[default]
    Auto,
    /// MILLIE_HOME/.credentials.json
    /// This file will be readable to Millie and other applications running as the same user.
    File,
...
/// Settings that govern if and what will be written to `~/.millie/history.jsonl`.
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Millie product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Millie product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Millie rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Millie does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
// ===== llama.cpp local provider configuration =====

/// Settings for the built-in `llamacpp` OSS provider: where the GGUF model
/// lives, which port `llama-server` should listen on, and which GPU(s) it
/// should be launched against. All fields are optional; CLI flags
/// (`--llama-model-path`, `--llama-gpu`, `--llama-port`) override these.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Default, JsonSchema)]
#[schemars(deny_unknown_fields)]
#[serde(deny_unknown_fields)]
pub struct LlamaCppConfigToml {
    /// Absolute path to the `.gguf` model file to serve.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub model_path: Option<AbsolutePathBuf>,

    /// Any Hugging Face GGUF to serve instead of a catalog model, as
    /// "org/repo/file.gguf". Downloaded to the model cache on first use.
    /// Ignored when `model_path` is set.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub model_hf: Option<String>,

    /// Port `llama-server` listens on (and where Millie looks for an
    /// already-running instance before launching a new one).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub port: Option<u16>,

    /// GPU indices to expose to `llama-server` via `CUDA_VISIBLE_DEVICES`,
    /// e.g. `[0]` (default if unset) or `[0, 1]`. Empty means CPU-only.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gpu: Option<Vec<u32>>,

    /// Context size passed to `llama-server --ctx-size`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ctx_size: Option<u32>,

    /// Number of server slots (concurrent requests), letting several Millie
    /// instances share one server without queueing behind each other.
    /// `ctx_size` is divided among slots, so scale it up accordingly.
    /// Server default is automatic.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub parallel: Option<u32>,

    /// Vision tower (mmproj) GGUF served alongside `model_path` for image
    /// input. Catalog models set this up automatically; this is for explicit
    /// `model_path` setups.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mmproj_path: Option<AbsolutePathBuf>,

    /// Whether to serve the vision tower (image input). Unset follows the
    /// machine's serving profile; `false` forces text-only, `true` forces
    /// vision on even where the profile would leave it off to save memory.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub vision: Option<bool>,

    /// KV cache type for both K and V: "q8_0" (the default; halves KV
    /// memory, needs flash attention, which is enabled with it), "f16"
    /// (server defaults), or "q4_0" (quarter memory; largely untested).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub kv_cache: Option<String>,

    /// Whether to memory-map model weights (`false` passes --no-mmap and
    /// loads everything into RAM). Unset follows the serving profile.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mmap: Option<bool>,

    /// Number of MoE layers whose experts stay on the CPU (llama-server
    /// --n-cpu-moe) while attention and dense weights run on the GPU, for
    /// hybrid GPU+CPU serving. Unset follows the serving profile.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub n_cpu_moe: Option<u32>,

    /// Pin the serving profile by name (e.g. "cpu-compact") instead of
    /// letting machine detection choose one. Individual settings above
    /// still override whatever the pinned profile defaults.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub profile: Option<String>,

    /// Path to the `llama-server` binary. Defaults to `llama-server` on PATH.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub server_bin: Option<AbsolutePathBuf>,

    /// Sampling temperature. When unset, uses the selected model's catalog default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub temperature: Option<f32>,

    /// Min-p sampling threshold. When unset, uses the selected model's catalog default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub min_p: Option<f32>,

    /// Top-k sampling cutoff. When unset, uses the selected model's catalog default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub top_k: Option<i32>,

    /// Top-p (nucleus) sampling cutoff. When unset, uses the selected model's catalog default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub top_p: Option<f32>,

    /// Per-completion generation cap (`max_tokens`). Defaults to 8192 when
    /// unset. When a completion hits this cap, the truncated output stays in
    /// the conversation and the model receives an error telling it to work
    /// in smaller steps.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_output_tokens: Option<u32>,

    /// Max tokens the model may spend thinking per completion; the server
    /// force-closes the thinking block when the budget is hit. Defaults to
    /// 1024 when unset; 0 disables the budget.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub thinking_budget: Option<u32>,

    /// Ask the server to stop a completion that is looping (one stretch of
    /// text repeated at least three times covering most of the last 300 to
    /// 900 tokens). Defaults to true.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub repeat_stop: Option<bool>,

    /// Clean retries per pathology episode (loop, repeated tool call, or
    /// token-cap hit): the failed completion is discarded and the identical
    /// request sampled again before any repetition penalty engages.
    /// 0 (default) disables resampling.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub resamples: Option<u32>,

    /// After a looped completion or a repeated tool call, the text that
    /// repeated is penalized on the next completion: every token that would
    /// extend a match of `ngram_penalty_start_n` or more tokens loses
    /// `(match length - start_n + 1) * scale` from its logit, capped at
    /// `ngram_penalty_max`. Defaults: scale 1.0, step 0.5, start_n 3, max 10.
    /// `ngram_penalty_scale = 0` disables the penalty.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ngram_penalty_scale: Option<f32>,

    /// Added to the scale for each further consecutive failing completion.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ngram_penalty_step: Option<f32>,

    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ngram_penalty_start_n: Option<u32>,

    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ngram_penalty_max: Option<f32>,

    /// When a completion is stopped for looping, drop it from the
    /// conversation (leaving a one-line note) and let the model retry under
    /// the penalty, instead of keeping the looped text. Defaults to true.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub backtrack_on_repetition: Option<bool>,
}

/// How harness notes are delivered to the model (`[tools] harness_notes`).
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Default, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum HarnessNotesMode {
    /// As user messages.
    User,
    /// As tool results with a synthetic call id (default).
    #[default]
    Tool,
}

/// Settings for the `vllm` provider: an OpenAI-compatible vLLM server serving a
/// Millie model, started separately (Millie only attaches to it). Generation
/// settings (sampling, token caps, thinking budget, loop guard) are the
/// `[llamacpp]` keys, which apply to both local backends.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Default, JsonSchema)]
#[schemars(deny_unknown_fields)]
#[serde(deny_unknown_fields)]
pub struct VllmConfigToml {
    /// Base URL of the server's OpenAI-compatible API, e.g.
    /// `http://127.0.0.1:8300/v1` (the default port is 8300).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub base_url: Option<String>,

    /// The `--served-model-name` the server was started with. Defaults to `millie`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Fallback resize-reflow row cap when Millie cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$MILLIE_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against MILLIE_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Millie's terminal-specific default. Set to `0` to
...
/// (primarily the Millie IDE extension). NOTE: these are different from
...
pub struct PluginMcpServerConfig {
    /// When `false`, Millie skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Millie successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Millie last successfully activated for this marketplace.
    #[serde(default)]
...
/// `[tools] repeat_guard`: what to do when the model issues the same tool call
/// again (identical to the previous call, or three times in the last ten).
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Default, JsonSchema)]
#[serde(rename_all = "kebab-case")]
pub enum RepeatGuardMode {
    /// Run the call and append a warning to its output (default).
    #[default]
    Warn,
    /// Run the call and only record the repeat in the model-call log.
    Log,
    /// Do nothing.
    Off,
}

/// `[tools] str_replace_autocorrect`: what to do when a `str_replace` fails
/// because `old_str` was not found but a single, statistically safe match
/// exists in the file.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Default, JsonSchema)]
#[serde(rename_all = "kebab-case")]
pub enum StrReplaceAutocorrectMode {
    /// Apply the correction, and tell the model what was changed.
    Correct,
    /// Return the original failure, but record the would-be correction in
    /// the model-call log (default).
    #[default]
    Log,
    /// Do nothing.
    Off,
...
/// Effective `[str_replace_autocorrect]` settings (defaults filled in).
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct StrReplaceAutocorrectSettings {
    pub mode: StrReplaceAutocorrectMode,
    /// Accept a correction only if the best candidate's posterior is at least this.
    pub posterior_threshold: f64,
    /// Never correct if the best candidate differs by more than this fraction of old_str.
    pub max_distance_fraction: f64,
    /// Prior mass on "the intended text is not in this file".
    pub h0_prior: f64,
    /// Decline for old_str longer than this many characters.
    pub max_old_str_chars: usize,
    /// Decline for files larger than this many bytes.
    pub max_file_bytes: usize,
}

impl Default for StrReplaceAutocorrectSettings {
    fn default() -> Self {
        Self {
            mode: StrReplaceAutocorrectMode::Log,
...
#[cfg(test)]
mod vllm_settings_tests {
    #[test]
    fn misplaced_sampling_is_an_error() {
        for key in ["temperature", "min_p", "top_p", "top_k"] {
            let value = format!("{key} = 0.5");
            assert!(toml::from_str::<super::VllmConfigToml>(&value).is_err());
        }
        let settings: super::VllmConfigToml =
            toml::from_str("base_url = 'http://localhost:8300/v1/'\nserved_model = 'millie'")
                .unwrap();
        assert_eq!(settings.served_model.as_deref(), Some("millie"));
    }
}
