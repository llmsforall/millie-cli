...
#[clap(
    author,
    // Identify as the product, not the workspace crate. The string is
    // assembled in build.rs: MILLIE_VERSION verbatim for release builds,
    // otherwise "<workspace version>-dev+g<commit>[.dirty]".
    name = "millie",
    version = env!("MILLIE_VERSION_STRING"),
    // If a sub‑command is given, ignore requirements of the default args.
    subcommand_negates_reqs = true,
    // The executable is sometimes invoked via a platform‑specific name, but
    // the help output should always use the generic command name users run.
    bin_name = "millie",
    override_usage = "millie [OPTIONS] [PROMPT]\n       millie [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Check for and install updated model weights from Hugging Face.
    Models(models::ModelsCommand),
    /// Run Millie non-interactively.
    ...
    /// Manage external MCP servers for Millie.
    Mcp(McpCli),

    /// Manage Millie plugins.
    Plugin(PluginCli),

    /// Start Millie as an MCP server (stdio).
    ...
    /// Diagnose local Millie installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Millie-provided sandbox.
...
struct AppServerCommand {
    ...
    /// ```
    ///
    /// See https://llmsforall.com for more details.
...
struct ExecServerCommand {
    ...
    /// Use Agent Identity auth from MILLIE_ACCESS_TOKEN for remote registration.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Millie tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Millie via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Millie.");
    Ok(())
}
...
fn main() -> anyhow::Result<()> {
    #[cfg(unix)]
    if std::env::args().nth(1).as_deref() == Some(codex_exec::MODEL_SUPERVISOR_ARG) {
        // Run outside the owning terminal's session; owner-pipe EOF governs cleanup.
        if unsafe { libc::setsid() } < 0 {
            return Err(std::io::Error::last_os_error().into());
        }
    }
    if let Some(result) = codex_exec::dispatch_model_supervisor() {
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Models(command)) => {
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
                "models",
            )?;
            models::run(command, root_config_overrides.clone(), &interactive).await?;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `millie-cli mcp` and `millie-cli models`: `millie-cli`, `millie-cli exec`, `millie-cli review`, `millie resume`, `millie-cli fork`, `millie-cli mcp`, `millie-cli sandbox`, `millie-cli models`, and `millie-cli debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let codex_self_exe = arg0_paths
        .codex_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Millie executable path is not configured"))?;
...
async fn load_exec_server_remote_auth_provider(
    ...
    if use_agent_identity_auth {
        let agent_identity_jwt = read_codex_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("MILLIE_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        let auth =
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires API key authentication; set MILLIE_API_KEY",
    )
    .await?;

    if !is_supported_exec_server_remote_auth(&auth) {
        anyhow::bail!(
            "remote exec-server registration requires cloud-account authentication or API key authentication; Agent Identity auth requires --use-agent-identity-auth"
        );
    }
...
async fn run_debug_prompt_input_command(
    ...
    let overrides = ConfigOverrides {
        ...
        simple_context: shared.simple_context.then_some(true),
        model_context_window: shared.model_context_window,
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `millie-cli {subcommand}`");
    }
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Millie's interactive TUI may not work in this terminal."
        );
        ...
        match local_state_db::repair_files(startup_error).await {
            ...
            Err(repair_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to repair Millie local data automatically: {repair_err}"
                )));
            }
        }
...
mod tests {
    ...
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["millie", "--profile", "work", "models", "check-updates"])
                .expect("model updates support named profiles")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        assert!(
            help.contains("Usage: millie plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: millie plugin marketplace add"),
            ("list", "Usage: millie plugin marketplace list"),
            ("upgrade", "Usage: millie plugin marketplace upgrade"),
            ("remove", "Usage: millie plugin marketplace remove"),
        ] {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run millie resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run millie resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `millie-cli mcp`"
        );
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `millie-cli remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `millie-cli app-server proxy`"
        );
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("MILLIE_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("MILLIE_REMOTE_AUTH_TOKEN", |_| {
