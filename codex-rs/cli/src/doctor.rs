...
/// Options for building a local Millie diagnostic report.
///
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun millie-cli doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load Millie config")
}
...
fn config_overrides_from_interactive(
    ...
    ConfigOverrides {
        ...
        model_provider: interactive.oss_provider.clone(),
        codex_self_exe: arg0_paths.codex_self_exe.clone(),
        codex_linux_sandbox_exe: arg0_paths.codex_linux_sandbox_exe.clone(),
        main_execve_wrapper_exe: arg0_paths.main_execve_wrapper_exe.clone(),
        show_raw_agent_reasoning: None,
        additional_writable_roots: interactive.add_dir.clone(),
        simple_context: interactive.simple_context.then_some(true),
        model_context_window: interactive.model_context_window,
        ..Default::default()
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("MILLIE_MANAGED_BY_BUN").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "MILLIE_MANAGED_PACKAGE_ROOT",
    );
    ...
    if path_entries.len() > 1 {
        details.push(format!("PATH millie-cli entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH millie-cli #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Millie so the JS shim provides MILLIE_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("MILLIE_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("MILLIE_MANAGED_BY_NPM").is_none()
        && env::var_os("MILLIE_MANAGED_BY_BUN").is_none()
    {
        return false;
    }
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("MILLIE_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("MILLIE_HOME: {}", config.codex_home.display()));
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    match load_auth_dot_json(&config.codex_home, config.cli_auth_credentials_store_mode) {
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check.remediation(
                    "Provide a supported auth env var or clear the stale stored credentials.",
                );
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Millie credentials were found",
        )
        .details(details)
        .remediation("Provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or clear the stored credentials file."),
    }
}
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["MILLIE_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "MILLIE_HOME", &config.codex_home);
    ...
    if status == CheckStatus::Fail {
        check = check.remediation(
            "Back up MILLIE_HOME, then remove or repair the affected SQLite database.",
        );
    }
    check
}
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "MILLIE_HOME was resolved without config",
        )
        .detail(format!("MILLIE_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "MILLIE_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
mod tests {
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "MILLIE_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("MILLIE_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Millie."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Millie.".to_string())
        );
    }
