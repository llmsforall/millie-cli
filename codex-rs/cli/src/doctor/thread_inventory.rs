...
fn missing_state_db_check(scan: RolloutScan, details: Vec<String>) -> DoctorCheck {
    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start Millie with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start Millie with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    if !scan.scan_errors.is_empty() || !scan.malformed_names.is_empty() || scan.reached_scan_cap {
        check = check.issue(
            ...
            .remedy("Check file permissions and unexpected files under MILLIE_HOME sessions."),
        );
    }
    check
}
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart Millie"))
        }));
        assert!(
            check
                .details
