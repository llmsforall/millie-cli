...
fn codex_command(codex_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(codex_utils_cargo_bin::cargo_bin("millie")?);
    cmd.env("MILLIE_HOME", codex_home);
...
fn write_marketplace_source_with_manifest(source: &Path, marketplace_manifest: &str) -> Result<()> {
    ...
    std::fs::create_dir_all(source.join("plugins").join("sample").join(".millie-plugin"))?;
    std::fs::write(
        source
            .join(".agents")
            .join("plugins")
...
async fn plugin_add_reinstalls_from_configured_marketplace_snapshot() -> Result<()> {
    ...
    assert!(
        codex_home
            .path()
            .join("plugins/cache/debug/sample/1.2.3/.millie-plugin/plugin.json")
            .is_file()
    );
