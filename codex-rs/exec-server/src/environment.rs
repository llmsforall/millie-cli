...
pub const CODEX_EXEC_SERVER_URL_ENV_VAR: &str = "MILLIE_EXEC_SERVER_URL";
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `MILLIE_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `MILLIE_HOME/environments.toml` is present, it defines the configured
    ...
    /// reading user config files from `MILLIE_HOME`.
