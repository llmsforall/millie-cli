...
Agent Identity JWT in `MILLIE_ACCESS_TOKEN` can opt into that auth path with
...
Alternatively, API users can instead use `MILLIE_API_KEY`;
Codex sends it as a bearer token on the registration request. For example:

```sh
MILLIE_API_KEY="$OPENAI_API_KEY" \
codex exec-server \
  --remote ... \
