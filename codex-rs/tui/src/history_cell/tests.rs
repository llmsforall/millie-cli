...
fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
    ...
    assert_eq!(
        rendered,
        vec![
            "✔ You approved millie to".to_string(),
...
fn standalone_unix_update_available_history_cell_snapshot() {
    let cell =
        UpdateAvailableHistoryCell::new("9.9.9".to_string(), Some(UpdateAction::RerunMillieInstaller));
...
fn session_header_includes_reasoning_level_when_present() {
    ...
    // The "/model to change" hint was removed for the local build.
