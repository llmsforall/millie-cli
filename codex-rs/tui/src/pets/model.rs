...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under MILLIE_HOME,
...
impl Pet {
    ...
    /// has already been materialized into MILLIE_HOME; if callers skip the
...
fn load_builtin_pet(pet: catalog::BuiltinPet, codex_home: Option<&Path>) -> Result<Pet> {
    let codex_home = codex_home.context("MILLIE_HOME is not available")?;
