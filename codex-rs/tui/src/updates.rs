...
pub fn get_upgrade_version(config: &Config) -> Option<String> {
    // The update checker would phone external release channels
    // (GitHub/Homebrew/npm). This build never checks for updates.
...
async fn check_for_update(version_file: &Path, action: Option<UpdateAction>) -> anyhow::Result<()> {
    // Millie has a single release channel: its own install bundle. The
    // upstream brew/npm registries and the parent project's GitHub releases
    // describe a different product's versions, so they are never consulted.
    let _ = action;
    let latest_version = CODEX_CLI_VERSION.to_string();
