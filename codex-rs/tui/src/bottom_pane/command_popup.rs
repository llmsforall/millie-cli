...
mod tests {
    ...
    #[test]
    fn model_command_is_hidden() {
        // /model is not offered in the local build (the model is chosen at
        // launch); it must never appear as a suggestion.
        let mut popup = CommandPopup::new(CommandPopupFlags::default(), Vec::new());
        popup.on_composer_text_change("/model".to_string());
        assert!(
            popup.filtered_items().iter().all(|item| !matches!(
                item,
                CommandItem::Builtin(cmd) if cmd.command() == "model"
            )),
    ...
    fn filtered_commands_keep_presentation_order_for_prefix() {
        ...
        let cmds: Vec<String> = popup
            ...
            .collect();
        // /model and /memories are hidden in the local build.
        assert_eq!(cmds, vec!["mention".to_string(), "mcp".to_string()]);
    }
