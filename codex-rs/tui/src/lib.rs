...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let codex_home = find_codex_home().wrap_err("failed to resolve MILLIE_HOME")?;
...
pub async fn run_main(
    ...
    let codex_home = match find_codex_home() {
        ...
        Err(err) => {
            eprintln!("Error finding millie home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if cli.download {
        // Explicit download approval, independent of the remembered model choice.
        unsafe {
            std::env::set_var("MILLIE_DOWNLOAD_APPROVED", "1");
        }
    }
    let mut manually_selected_oss_provider = None;
    let model_provider_override = if cli.oss_provider.is_some() {
        ...
        Some(codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID)
            | Some(codex_model_provider_info::VLLM_OSS_PROVIDER_ID)
    ) {
        // `model_provider = "llamacpp"` (or "vllm") in config.toml selects the
        // provider on the request path, so the launch side (env exports below
        // and provider bootstrap) must run for it too -- same fix as in codex-exec.
        ...
        // provider; the launch side follows the same default (see codex-exec).
        Some(codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID.to_string())
    } else {
        None
    };

    // For the local llama.cpp provider, export the `[llamacpp]` config.toml
    // section as `CODEX_LLAMACPP_*` env vars, mirroring the same block in
    // codex-exec: `codex-llamacpp::ensure_oss_ready` reads these when the
    // provider is bootstrapped.
    if model_provider_override.as_deref() == Some(codex_model_provider_info::VLLM_OSS_PROVIDER_ID) {
    ...
    if matches!(
        ...
        unsafe {
            std::env::set_var(
                ...
                codex_home.join("llama-server.log"),
            );
            std::env::set_var("MILLIE_LLAMACPP_CACHE_BASE", codex_home.join("kvcache"));
        }
        if let Some(path) = cli
            .llama_model_path
            .clone()
            .or(llamacpp_toml.model_path.map(|p| p.to_path_buf()))
        {
        ...
        if let Some(gpu) = cli.llama_gpu.clone().or(llamacpp_toml.gpu) {
        ...
        if let Some(port) = cli.llama_port.or(llamacpp_toml.port) {
            unsafe {
        ...
        let vision_override = if cli.vision {
            Some(true)
        } else if cli.no_vision {
            Some(false)
        } else {
    ...
    // Default the agent-side context window to the server's default ctx_size
    // so compaction thresholds track the KV budget.
    let model_context_window_with_default = cli.model_context_window.or_else(|| {
        (matches!(
            model_provider_override.as_deref(),
            Some(codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID)
                | Some(codex_model_provider_info::VLLM_OSS_PROVIDER_ID)
        ) && config_toml.model_context_window.is_none())
        .then_some(codex_llamacpp::DEFAULT_CTX_SIZE as i64)
    });

    let model = if model_provider_override.as_deref()
        == Some(codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID)
        || (model_provider_override.as_deref()
            == Some(codex_model_provider_info::VLLM_OSS_PROVIDER_ID)
            && cli.model.as_deref().is_some_and(|model| model != "select"))
    {
    ...
    let overrides = ConfigOverrides {
        ...
        simple_context: cli.simple_context.then_some(true),
        model_context_window: model_context_window_with_default,
...
/// This build has no hosted-account login, so it never shows a login screen.
const HOSTED_LOGIN_ENABLED: bool = false;
