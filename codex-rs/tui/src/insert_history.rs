...
/// Zellij does not constrain soft-wrapped continuation rows to Millie's scroll region, so its raw
...
mod tests {
    ...
    fn vt100_terminal_wrap_policy_does_not_pre_wrap_long_paragraph() {
        ...
        assert!(
            ...
            "expected terminal soft-wrap instead of Millie word pre-wrap, rows: {rows:?}"
        );
    }
