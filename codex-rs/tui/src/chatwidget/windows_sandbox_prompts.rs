...
impl ChatWidget {
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up the Millie agent sandbox to protect your files and control network access."],
            ])
            .wrap(Wrap { trim: false }),
        ));
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        lines.push(line![""]);
        lines.push(line![
            "You can still use Millie in a non-admin sandbox. It carries greater risk if prompt injected."
        ]);
        lines.push(line![
            "Learn more <https://llmsforall.com>"
        ]);
        ...
        let items = vec![
            ...
            SelectionItem {
                name: "Use Millie with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
