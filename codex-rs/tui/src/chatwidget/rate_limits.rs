...
/// The model the rate-limit nudge suggests switching to: the first entry of
/// the runtime model catalog (no model name is compiled in).
#[cfg(test)]
pub(super) const NUDGE_MODEL_SLUG: &str = "millie-35B-A3B-7GB";
...
impl ChatWidget {
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            if high_usage
                ...
                && self
                    .lower_cost_preset()
                    .is_some_and(|preset| preset.model != self.current_model())
                && !matches!(
    ...
    fn lower_cost_preset(&self) -> Option<ModelPreset> {
        ...
        models.iter().find(|preset| preset.show_in_picker).cloned()
    }
