...
impl ChatWidget {
    /// Keep a local session's model identity tied to the server selected at startup.
    pub(crate) fn local_model_selection_is_startup_only(&mut self) -> bool {
        if self.config.model_provider.name != codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID {
            return false;
        }
        self.add_info_message(
            "Local models are selected at startup. Close existing Millie sessions, then run `millie --model select`.".to_string(),
            /*hint*/ None,
        );
        true
    }
    ...
    pub(crate) fn open_model_popup(&mut self) {
        if self.local_model_selection_is_startup_only() {
            return;
        }
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        ...
        let warning = format!(
            "Warning: the model provider base URL is overridden to {base_url}. Selecting models may not be supported or work properly."
        );
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Select a model by running millie-cli -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        for choice in choices.iter() {
            ...
            let selected_description: Option<String> = None;
