...
pub(super) async fn make_chatwidget_manual(
    ...
    widget.normal_placeholder_text = "Ask Millie to do anything".to_string();
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    // millie defaults to the llamacpp provider, which needs no OpenAI auth;
    // this fixture simulates a ChatGPT-authed setup, so make the provider
    // consistent with that
    chat.config.model_provider.requires_openai_auth = true;
    chat.model_catalog = test_model_catalog(&chat.config);
}

/// The rate-limit nudge only renders when the catalog contains
/// NUDGE_MODEL_SLUG; the upstream fixture catalog does not carry it.
pub(crate) fn add_nudge_model_to_test_catalog(chat: &mut ChatWidget) {
    let mut models = chat.model_catalog.try_list_models().expect("list models");
    models.push(
        test_model_info(
            crate::chatwidget::rate_limits::NUDGE_MODEL_SLUG,
            /*priority*/ 99,
