...
#[tokio::test]
async fn local_model_menu_requires_startup_selection() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.model_provider.name =
        codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID.to_string();
    chat.thread_id = Some(ThreadId::new());
    let selected = chat.current_model().to_string();
    chat.open_model_popup();
    assert_eq!(chat.current_model(), selected);
    let rendered = drain_insert_history(&mut rx)
        .into_iter()
        .map(|lines| lines_to_single_string(&lines))
        .collect::<Vec<_>>();
    insta::assert_snapshot!("local_model_startup_selection", rendered.join("\n\n"));
}
...
#[tokio::test]
async fn model_reasoning_selection_popup_never_warns_about_plan_limits() {
    // millie models run locally with no metered plan; the upstream
    // "consumes Plus plan rate limits" warning was removed outright
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.2")).await;
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert!(
        !popup.contains("rate limits"),
        "reasoning popup must not warn about plan rate limits: {popup}"
    );
}
