//! Asynchronous worker that executes a **Millie** tool-call inside a spawned
...
/// To adhere to MCP `tools/call` response format, include the Millie
...
/// Run a complete Millie session and stream events back to the client.
///
...
pub async fn run_codex_tool_session(
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start Millie session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the Millie submission so that
