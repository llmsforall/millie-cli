...
fn write_plugin_skill(
    ...
    fs::create_dir_all(plugin_root.join(".millie-plugin")).unwrap();
    fs::create_dir_all(&skill_dir).unwrap();
    fs::write(
...
#[test]
fn disabling_bundled_skills_keeps_the_files_on_disk() {
    // Disabling bundled skills is a prompt-token choice, not a request to
    // delete files: the sample skills stay installed under skills/.system so
    // they can be inspected and re-enabled; they are simply not advertised in
    // the prompt (enforced by root selection, not by removal).
    ...
    assert!(
        ...
        "disabling bundled skills must not delete the installed sample skills"
    );
}
