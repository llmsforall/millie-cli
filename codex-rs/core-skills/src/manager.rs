...
use codex_config::BundledSkillsConfig;
...
impl SkillsManager {
    ...
    pub fn new_with_restriction_product(
        codex_home: AbsolutePathBuf,
        // Retained for API compatibility; whether bundled skills appear in the
        // prompt is now decided by root selection from the config stack, and
        // the sample files are always installed on disk regardless.
        ...
        // Always keep the bundled sample skills on disk under `skills/.system`
        // so they are available to inspect and to enable. Whether they appear
        // in the prompt is decided by root selection from the config, not by
        // the presence of the files -- disabling them is a token-cost choice,
        // not a request to delete the user's files.
...
pub fn bundled_skills_enabled_from_stack(
    ...
    // No `[skills]` table, or an unparsable one, means the user has not opted
    // in -- bundled sample skills stay out of the prompt (see
    // BundledSkillsConfig::default).
    let Some(skills_value) = effective_config
        .as_table()
        .and_then(|table| table.get("skills"))
    else {
        return BundledSkillsConfig::default().enabled;
    };
