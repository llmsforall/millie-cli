...
impl Codex {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        let session_configuration = SessionConfiguration {
            ...
            extension_tools_only: config.extension_tools_only,
            base_instructions,
...
impl Session {
    ...
    /// Warm the local model server's prompt cache for the current
    /// conversation, in the background, so the next turn (or the first turn
    /// after a resume) reuses the whole history instead of re-encoding it.
    ///
    /// Needed because the local model is a hybrid SSM+attention model whose
    /// cached state cannot be partially trimmed: when past-turn reasoning is
    /// stripped between turns, the cached state diverges early and the entire
    /// conversation is otherwise re-prefilled. Prefilling the stripped
    /// history during idle time moves that cost off the user's critical path.
    ///
    /// Local (`llamacpp`) provider only; fire-and-forget and best-effort.
    /// Build the (server base URL, chat messages) for a prompt-cache warm-up of
    /// the current think-stripped history, or `None` when there is nothing to
    /// warm (not the local provider, no base URL, or empty history).
    async fn prewarm_request(
        &self,
        turn_context: &TurnContext,
    ) -> Option<(String, Vec<serde_json::Value>)> {
        let provider = self.provider().await;
        if provider.name != codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID {
            return None;
        }
        let base_url = provider.base_url.as_deref()?;
        // apply-template / completion live at the server root, not under /v1.
        let base_url = codex_model_provider_info::local_server_root(base_url).to_string();

        let instructions = self.get_base_instructions().await;
        let history = self.clone_history().await;
        let items = history.for_prompt(&turn_context.model_info.input_modalities);
        // Nothing to warm beyond the system prompt (already cached separately).
        if items.is_empty() {
            return None;
        }
        let messages = codex_llamacpp::build_chat_messages(&instructions.text, &items);
        Some((base_url, messages))
    }

    /// Acquire the prewarm lock at the very start of turn teardown, but only for
    /// the local provider. Held across teardown so that a next turn arriving
    /// while we still owe it a warm cache blocks at the `start_task` barrier
    /// immediately -- before it can set the active turn or issue a competing
    /// request -- instead of racing the warm-up. `None` for remote providers
    /// (nothing to serialize).
    pub(crate) async fn acquire_prewarm_guard_if_local(
        &self,
    ) -> Option<tokio::sync::OwnedMutexGuard<()>> {
        let provider = self.provider().await;
        if provider.name == codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID {
            let guard = Arc::clone(&self.prewarm_lock).lock_owned().await;
            *self.prewarm_cancel.lock().await = tokio_util::sync::CancellationToken::new();
            Some(guard)
        } else {
            None
        }
    ...
    /// Run the warm-up inline to completion. The caller must already hold the
    /// prewarm guard (see [`acquire_prewarm_guard_if_local`]). This is the
    /// mandatory turn-teardown path: it always runs and always finishes before
    /// the session accepts the next turn, so the cache is deterministically left
    /// in the think-stripped form the next turn's prefix matches directly.
    pub(crate) async fn prewarm_prompt_cache_blocking(&self, turn_context: &TurnContext) {
        let Some((base_url, messages)) = self.prewarm_request(turn_context).await else {
            return;
        };
        let cancel = self.prewarm_cancel.lock().await.clone();
        tokio::select! {
            _ = cancel.cancelled() => {},
            _ = codex_llamacpp::warm_conversation_cache(&base_url, messages) => {},
        }
    }

    /// Background warm-up used by the resume path, where there is no turn
    /// teardown to run it inline. Takes the prewarm lock before spawning and
    /// moves the guard into the task so it is held for the whole prefill with no
    /// gap; the first real turn blocks at the `start_task` barrier until it
    /// finishes rather than racing it on the single-slot server.
    pub(crate) async fn spawn_prompt_cache_warmup(&self, turn_context: &TurnContext) {
        let Some((base_url, messages)) = self.prewarm_request(turn_context).await else {
            return;
        };
        let guard = Arc::clone(&self.prewarm_lock).lock_owned().await;
        let cancel = tokio_util::sync::CancellationToken::new();
        *self.prewarm_cancel.lock().await = cancel.clone();
    ...
    async fn record_initial_history(&self, conversation_history: InitialHistory) {
        ...
        match conversation_history {
            ...
            InitialHistory::Resumed(resumed_history) => {
                ...
                // A resumed conversation has nothing cached on the local
                // server, so its first message would otherwise re-encode the
                // whole history. Warm it in the background now.
                if has_prior_user_turns && !is_subagent {
                    self.spawn_prompt_cache_warmup(&turn_context).await;
                }
            }
    ...
    /// Number of items in the conversation history right now.
    pub(crate) async fn history_len(&self) -> usize {
        let state = self.state.lock().await;
        state.clone_history().raw_items().len()
    }

    /// Drop every history item recorded after the first `len` items (loop
    /// guard backtracking). The rollout file keeps what was dropped.
    pub(crate) async fn truncate_history(&self, len: usize) -> usize {
        let mut state = self.state.lock().await;
        let mut items = state.clone_history().into_raw_items();
        if items.len() <= len {
            return 0;
        }
        let dropped = items.len() - len;
        items.truncate(len);
        let reference = state.reference_context_item();
        state.replace_history(items, reference);
        dropped
    }
    ...
    pub(crate) async fn record_user_prompt_and_emit_turn_item(
        ...
        // A new accepted user message can legitimately ask for the same tool again.
        // Reset here (also for steered input), not for synthetic continuation/context items.
        if !input.is_empty()
            && let Ok(mut recent) = self.recent_tool_calls.lock()
        {
            recent.clear();
        }
