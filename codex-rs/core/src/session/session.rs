...
pub(crate) struct Session {
    ...
    /// Tool calls since the latest accepted user message, for the repeat guard.
    pub(crate) recent_tool_calls:
        std::sync::Mutex<std::collections::VecDeque<crate::tools::repeat_guard::RecentToolCall>>,
    /// Loop-guard state for the llamacpp provider (see `crate::pathology`).
    pub(crate) pathology: std::sync::Mutex<crate::pathology::PathologyState>,
    pub(crate) resample: std::sync::Mutex<crate::pathology::ResampleState>,
    ...
    /// Serializes the background prompt-cache warm-up against real turns on the
    /// single-slot local model server. The warm-up task holds this for its
    /// duration; each turn acquires it once at start (as a barrier) so a turn
    /// arriving mid-warm-up waits for the warm-up to finish and then reuses the
    /// prefilled cache, instead of racing it and corrupting the recurrent state.
    pub(crate) prewarm_lock: Arc<Mutex<()>>,
    pub(crate) prewarm_cancel: Mutex<tokio_util::sync::CancellationToken>,
}
...
pub(crate) struct SessionConfiguration {
    ...
    /// Restrict the tool surface to extension-contributed tools only.
    pub(super) extension_tools_only: bool,
...
impl SessionConfiguration {
    ...
    pub(super) fn thread_config_snapshot(&self) -> ThreadConfigSnapshot {
        ThreadConfigSnapshot {
            ...
            extension_tools_only: self.extension_tools_only,
        }
    }
    ...
    pub(crate) async fn new(
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let sess = Arc::new(Session {
                ...
                prewarm_lock: Arc::new(Mutex::new(())),
                prewarm_cancel: Mutex::new(tokio_util::sync::CancellationToken::new()),
            });
