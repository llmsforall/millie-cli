...
async fn auto_compact_token_status(
    ...
    let (auto_compact_scope_tokens, auto_compact_scope_limit, full_context_window_limit) =
        if turn_context.config.simple_context {
            // Flat headroom instead of the usual 90%-of-context-window clamp: there's no
            // multiplicative safety margin here, so the constant is intentionally larger than
            // the one used for the retention budget below.
            let context_window = turn_context
                .effective_raw_context_window()
                .unwrap_or(i64::MAX);
            (
                active_context_tokens,
                context_window.saturating_sub(4096),
                None,
            )
        } else {
                ...
                AutoCompactTokenLimitScope::BodyAfterPrefix => {
                    ...
                    (
                        ...
                        turn_context
                            ...
                            .or_else(|| turn_context.effective_auto_compact_token_limit())
                            .unwrap_or(i64::MAX),
...
async fn run_sampling_request(
    ...
    loop {
        ...
        if crate::llamacpp_chat_transport::is_local_backend(&turn_context.provider.info().name) {
            // Loop guard: repetition stop on every request, n-gram penalty
            // after a pathology (see crate::pathology).
            let params = codex_llamacpp::pathology_params_from_env();
            prompt.llamacpp_request_extra = sess.pathology.lock().ok().and_then(|mut st| {
                st.begin_call();
                st.request_extra(&params)
            });
        }
        ...
        if turn_context.provider.info().name == codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID
        {
            let message = err.to_string();
            if message.contains("llama-server") {
                return Err(CodexErr::Fatal(format!(
                    "{message}\nModel server request stopped. To reopen this conversation and attempt to reload the model server, run: millie resume {}",
...
async fn try_run_sampling_request(
    ...
    // Where the history stood before this completion, for loop-guard backtracking.
    let history_mark = sess.history_len().await;
    ...
    let mut last_agent_message: Option<String> = None;
    let mut completion_reasoning = String::new();
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        match event {
            ...
            ResponseEvent::OutputItemDone(item) => {
                ...
                {
                    for part in parts {
                        let (ReasoningItemContent::ReasoningText { text }
                        | ReasoningItemContent::Text { text }) = part;
                        completion_reasoning.push_str(text);
                    }
                }
            ...
            ResponseEvent::Completed {
                ...
                let malformed_call = finish_reason.as_deref()
                    == Some(crate::llamacpp_chat_transport::MALFORMED_TOOL_CALL_FINISH_REASON);

                // Loop guard (llamacpp provider): a completion the server
                // stopped for repeating, or one that hit the cap while
                // repeating, is a failure: penalize the repeated text on the
                // next completion and, by default, discard the looped output.
                let is_llamacpp = crate::llamacpp_chat_transport::is_local_backend(
                    &turn_context.provider.info().name,
                );
                let repetition_stats = is_llamacpp
                    .then(|| {
                        crate::pathology::take_repetition_stats(
                            &sess.services.model_client.thread_id_string(),
                            sess.services.model_client.last_model_call_ordinal(),
                        )
                    })
                    .flatten();
                let loop_span = if is_llamacpp {
                    crate::pathology::loop_span(finish_reason.as_deref(), repetition_stats.as_ref())
                } else {
                    None
                };
                // Resample layer: the first line of defense against every
                // pathology (loop, token-cap hit, repeated tool call). While
                // budget remains, the failed completion is discarded and the
                // identical request re-issued -- fresh seed, no penalties, no
                // injected notes, and the penalty machinery's state untouched.
                // Only an exhausted budget falls through to the flow below.
                if is_llamacpp {
                    let params = codex_llamacpp::pathology_params_from_env();
                    let requested = sess
                        .resample
                        .lock()
                        .map(|mut r| r.take_requested())
                        .unwrap_or(false);
                    let trigger = if requested {
                        Some("repeated_tool_call")
                    } else if loop_span.is_some() {
                        Some("loop")
                    } else if truncated {
                        Some("cap")
                    } else {
                    ...
                    if let Some(trigger) = trigger {
                        // Repeated-call resamples were claimed at dispatch;
                        // loop/cap claim here.
                        let claimed = requested
                            || sess
                                .resample
                                .lock()
                                .map(|mut r| r.try_claim(params.resamples))
                                .unwrap_or(false);
                        if claimed {
                            let attempt = sess.resample.lock().map(|r| r.used()).unwrap_or(0);
                            in_flight.clear();
                            let dropped = sess.truncate_history(history_mark).await;
                            if crate::model_call_log::enabled() {
                                let key = crate::model_call_log::ModelCallKey {
                                    thread_id: sess.services.model_client.thread_id_string(),
                                    turn_id: turn_context.sub_id.clone(),
                                    call_ordinal: sess
                                        .services
                                        .model_client
                                        .last_model_call_ordinal(),
                                };
                                let discarded: String = last_agent_message
                                    .as_deref()
                                    .unwrap_or("")
                                    .chars()
                                    .take(2000)
                                ...
                                crate::model_call_log::record_event(
                                    ...
                                    serde_json::json!({
                                        ...
                                        "trigger": trigger,
                                        "attempt": attempt,
                                        "budget": params.resamples,
                                        "finish_reason": finish_reason,
                                        "repetition": repetition_stats,
                                        "dropped_items": dropped,
                                        "discarded_message": discarded,
                                    }),
                                );
                            }
                            sess.send_event(
                                ...
                                EventMsg::Warning(WarningEvent {
                                    message: format!(
                                        "Model output failed ({trigger}); resampling (attempt {attempt} of {}).",
                                        params.resamples
                                    ),
                                }),
                            )
                ...
                if let Some(span) = loop_span {
                    let params = codex_llamacpp::pathology_params_from_env();
                    let state_summary = sess.pathology.lock().ok().map(|mut st| {
                        st.on_failure(crate::pathology::Trigger::Loop, vec![span.clone()], &params);
                        st.summary()
                    });
                    let backtrack = params.backtrack_on_repetition;
                    let dropped = if backtrack {
                        // Abort this completion's tool calls and drop what it recorded.
                        in_flight.clear();
                        sess.truncate_history(history_mark).await
                    } else {
                        0
                    };
                    if crate::model_call_log::enabled() {
                        let key = crate::model_call_log::ModelCallKey {
                            thread_id: sess.services.model_client.thread_id_string(),
                            turn_id: turn_context.sub_id.clone(),
                            call_ordinal: sess.services.model_client.last_model_call_ordinal(),
                        };
                        crate::model_call_log::record_event(
                            "pathology",
                            &key,
                            None,
                            serde_json::json!({
                                "trigger": crate::pathology::Trigger::Loop.as_str(),
                                "finish_reason": finish_reason,
                                "repetition": repetition_stats,
                                "backtrack": backtrack,
                                "dropped_items": dropped,
                    ...
                    let shown: String = span.chars().take(200).collect();
                    let shown = if span.chars().count() > 200 {
                        format!("{shown}...")
                    } else {
                        shown
                    };
                    sess.send_event(
                        &turn_context,
                        EventMsg::Warning(WarningEvent {
                            message: if backtrack {
                                "Model output looped; discarded it and retrying with a repetition penalty."
                                    .to_string()
                            } else {
                                "Model output looped; continuing with a repetition penalty.".to_string()
                            },
                        }),
                    )
                    .await;
                    let prompt_text = if backtrack {
                        "[Your previous attempt at this step repeated the same text over and over and was \
                         discarded. Take a different approach.]"
                            .to_string()
                    } else {
                        format!(
                            "ERROR: your output was stopped because it kept repeating the same text \
                             (repeated: {shown:?}). Everything before the stop is preserved above. \
                             Continue the task without repeating it."
                        )
                    };
                    let message = harness_note_item(prompt_text, turn_context.config.harness_notes);
                ...
                if is_llamacpp && !truncated && !malformed_call {
                    if let Ok(mut st) = sess.pathology.lock() {
                        st.note_clean_finish();
                    }
                    if let Ok(mut r) = sess.resample.lock() {
                        r.reset();
                    }
                }

                let continuation = if malformed_call {
                    Some(malformed_tool_call_continuation_prompt())
                } else {
                ...
                if let Some(prompt_text) = continuation {
                    if truncated {
                        sess.send_event(
                            &turn_context,
                            EventMsg::Warning(WarningEvent {
                                message: "Model output hit the generation token cap; continuing."
                                    .to_string(),
                            }),
                        )
                        .await;
...
/// Continuation prompt for a completion that contained a tool call whose
/// arguments were not complete JSON (and was therefore not executed). The
/// call's text is preserved in the assistant message by the transport.
/// A harness message to the model (continuation prompt, loop-guard note) as a
/// tool result rather than a user message. Chat templates of this model family
/// keep the current turn's reasoning only up to the last *user* message and
/// render tool results as `<tool_response>` blocks that do not move that
/// boundary, so as a tool result the note reaches the model with its reasoning
/// intact and in the shape environment feedback normally takes. The call id is
/// synthetic; no tool call precedes it.
fn harness_note_item(text: String, mode: codex_config::types::HarnessNotesMode) -> ResponseItem {
    if mode == codex_config::types::HarnessNotesMode::User {
    ...
    ResponseItem::FunctionCallOutput {
        call_id: format!(
            "{}{}",
            crate::context_manager::normalize::HARNESS_NOTE_CALL_PREFIX,
            uuid::Uuid::new_v4().simple()
        ),
        output: codex_protocol::models::FunctionCallOutputPayload {
            body: codex_protocol::models::FunctionCallOutputBody::Text(text),
            success: Some(false),
        },
    }
}

fn malformed_tool_call_continuation_prompt() -> String {
    "ERROR: your last tool call was incomplete or malformed (its text is preserved above, \
     marked \"incomplete tool call, not executed\"), so it was NOT executed. Make the call \
     again, complete and well-formed; if it was large, split the work into smaller calls."
        .to_string()
}

/// Decide whether the completion that just finished needs a continuation
/// prompt, and build it. These are continuations, not retries: the model's
/// output stays in history untouched, and the prompt below is appended as an
/// ordinary message for the next sampling request. Three conditions qualify:
///
/// 1. The completion was truncated at the generation token cap.
/// 2. The model wrote a tool call inside its reasoning (which no parser
///    executes). Reasoning is stripped from later prompts, so the prompt must
///    echo the call verbatim — it is the only surviving trace of what the
///    model tried to do. The call is deliberately never executed: the model
///    could not remember making it.
/// 3. The completion produced no tool call and no user-visible message.
///
/// Returns `None` when the completion needs no intervention.
fn turn_continuation_prompt(
    truncated: bool,
    made_tool_call: bool,
    ...
    if truncated {
        return Some(
            "ERROR: your output was cut off because it reached the generation token cap. \
             Everything you produced before the cutoff is preserved above. Perform the task \
             in smaller steps: shorter messages, smaller file writes, and smaller edits."
                .to_string(),
        );
    }
    if made_tool_call || agent_message.is_some_and(|m| !m.trim().is_empty()) {
        return None;
    }
    let call_start = reasoning
        .find("<tool_call>")
        .or_else(|| reasoning.find("<function="));
    if let Some(start) = call_start {
        let end = reasoning[start..]
            .find("</tool_call>")
            .map(|e| start + e + "</tool_call>".len())
            .unwrap_or(reasoning.len());
        let block = &reasoning[start..end.min(start + 4000)];
        return Some(format!(
            "ERROR: you wrote the following tool call inside your reasoning, so it was NOT \
             executed:\n\n{block}\n\nTool calls only execute when made after your reasoning, \
             never inside it. Make the tool call again now."
        ));
    }
    Some(
        "ERROR: your last response contained no message and no tool call, so nothing happened. \
         Reply with a message for the user or make a tool call."
            .to_string(),
    )
}
...
mod turn_continuation_tests {
    use super::turn_continuation_prompt;

    #[test]
    fn truncation_wins_and_asks_for_smaller_steps() {
        let p = turn_continuation_prompt(true, true, Some("partial"), "").unwrap();
        assert!(p.contains("generation token cap"));
        assert!(p.contains("smaller"));
    }

    #[test]
    fn tool_call_in_reasoning_is_echoed_verbatim_and_not_executed() {
        let reasoning = "I should run the test.\n<tool_call>\n<function=exec_command>\n<parameter=cmd>\npytest -v\n</parameter>\n</function>\n</tool_call>";
        let p = turn_continuation_prompt(false, false, None, reasoning).unwrap();
        assert!(p.contains("<function=exec_command>"));
        assert!(p.contains("pytest -v"));
        assert!(p.contains("NOT"));
    }

    #[test]
    fn silent_ending_gets_a_nudge() {
        let p = turn_continuation_prompt(false, false, None, "hmm, done I guess").unwrap();
        assert!(p.contains("no message and no tool call"));
    }

    #[test]
    fn normal_completions_are_untouched() {
        assert!(turn_continuation_prompt(false, true, None, "thinking").is_none());
        assert!(turn_continuation_prompt(false, false, Some("here you go"), "thinking").is_none());
        // quoting tool-call syntax in reasoning while ALSO answering properly is fine
        assert!(
            turn_continuation_prompt(false, false, Some("done"), "<tool_call> syntax…").is_none()
        );
    }
}
