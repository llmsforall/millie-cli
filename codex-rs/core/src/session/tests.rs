...
#[tokio::test]
async fn next_turn_cancels_conversation_cache_warmup_before_waiting_for_its_lock() {
    let (sess, tc, rx) = make_session_and_context_with_rx().await;
    let (_tx, startup_rx) = tokio::sync::oneshot::channel::<()>();
    ...
    .await;
    let guard = sess.prewarm_lock.clone().lock_owned().await;
    let cancellation = sess.prewarm_cancel.lock().await.clone();
    ...
    .await
    .expect("next turn must cancel warmup rather than wait for its HTTP timeout");
    warmup.await.unwrap();
...
#[tokio::test]
async fn interruption_cancels_conversation_cache_warmup_without_an_active_turn() {
    let (sess, _, _) = make_session_and_context_with_rx().await;
    let guard = sess.prewarm_lock.clone().lock_owned().await;
    let cancellation = sess.prewarm_cancel.lock().await.clone();
    let warmup = tokio::spawn(async move {
        let _guard = guard;
    ...
    sess.abort_all_tasks(TurnAbortReason::Interrupted).await;
    tokio::time::timeout(std::time::Duration::from_secs(2), warmup)
        .await
        .expect("interruption must release the warmup lock")
        .unwrap();
    assert!(sess.prewarm_lock.try_lock().is_ok());
}
...
#[test]
fn served_prompt_embeds_str_replace_doc_and_not_apply_patch() {
    // Checks the prompt the llamacpp path actually sends: the catalog entry's
    // rendered instructions, built from the prompt file -- not a compiled-in
    // default that the served model never sees.
    let str_replace_doc = include_str!("../../../../../str_replace_doc_v1.txt");
    let prompt = codex_models_manager::system_prompt::resolve_system_prompt_for_tests(
        std::path::Path::new("/nonexistent-millie-home"),
    );
    for model in codex_models_manager::bundled_millie_models(&prompt.text) {
        let served = model.get_model_instructions(/*personality*/ None);
        assert!(
            served.contains(str_replace_doc.trim()),
            "{}: served prompt should contain the verbatim str_replace doc",
            model.slug
        );
        assert!(
            !served.contains("apply_patch"),
            "{}: served prompt must not mention apply_patch",
            model.slug
        );
        assert!(
            !served.contains("{{ personality }}"),
            "{}: personality placeholder must be rendered",
            model.slug
        );
    }
}
...
async fn turn_context_with_model_updates_model_fields() {
    ...
    assert_eq!(updated.config.model.as_deref(), Some("millie-35B-A3B-11GB"));
    assert_eq!(updated.collaboration_mode.model(), "millie-35B-A3B-11GB");
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let session = Session {
        recent_tool_calls: Default::default(),
        pathology: Default::default(),
        ...
        prewarm_lock: std::sync::Arc::new(tokio::sync::Mutex::new(())),
        prewarm_cancel: tokio::sync::Mutex::new(tokio_util::sync::CancellationToken::new()),
    };
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let root_dot_codex = project_root.join(".millie");
    let nested_dot_codex = nested.join(".millie");
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_codex = project_root.join(".millie");
...
#[tokio::test]
async fn accepted_user_message_resets_tool_repeat_counts_even_within_the_same_turn() {
    use crate::tools::repeat_guard::RepeatVerdict;
    use crate::tools::repeat_guard::judge_and_remember;
    let (session, turn_context) = make_session_and_context().await;
    let session = Arc::new(session);
    let turn_context = Arc::new(turn_context);
    let signature = "read_file\0{\"path\":\"changed.txt\"}";
    let judge = |call_id: &str| {
        judge_and_remember(
            &mut session.recent_tool_calls.lock().unwrap(),
            call_id,
            signature,
            "changed.txt",
        )
    };
    assert!(!judge("before-1").triggered());
    assert!(judge("before-2").triggered());
    // Internal context can also have role=user; it must not clear the guard.
    ...
    .await;
    assert!(judge("before-3").triggered());
    // Keep the same TurnContext to exercise accepted input during an active turn,
    // as well as the recording path used for a fresh turn.
    crate::hook_runtime::record_pending_input(
        &session,
        &turn_context,
        TurnInput::UserInput {
            content: vec![UserInput::Text {
                text: "I changed changed.txt; read it again.".to_string(),
                text_elements: Vec::new(),
            }],
            client_id: None,
        },
