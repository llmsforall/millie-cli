...
impl TurnContext {
    ...
    /// The context size the running local llama-server actually serves,
    /// exported by the launcher once a server is confirmed healthy (spawned
    /// or reused). None for remote providers or before the local bootstrap
    /// ran. Read per call so a turn built before the server launch still
    /// picks it up on its next check.
    fn local_server_ctx(&self) -> Option<i64> {
        if self.provider.info().name != codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID {
            return None;
        }
        std::env::var(codex_llamacpp::ENV_RESOLVED_CTX)
            .ok()?
            .trim()
            .parse()
            .ok()
    ...
    /// The model's context window clamped to what the local server actually
    /// allocated. `model_context_window` stays the user's one knob; this
    /// feeds back the machine-fit profile / reused-server reality so
    /// compaction thresholds and token accounting track the real KV budget.
    pub(crate) fn effective_raw_context_window(&self) -> Option<i64> {
        let window = self.model_info.resolved_context_window();
        match (window, self.local_server_ctx()) {
            (Some(w), Some(s)) => Some(w.min(s)),
            (w, s) => w.or(s),
        }
    }

    /// `ModelInfo::auto_compact_token_limit` (90% of window, min'd with any
    /// configured limit) recomputed under the local-server clamp.
    pub(crate) fn effective_auto_compact_token_limit(&self) -> Option<i64> {
        let base = self.model_info.auto_compact_token_limit();
        match (base, self.local_server_ctx()) {
            (Some(b), Some(s)) => Some(b.min((s * 9) / 10)),
            (b, _) => b,
        }
    }
    ...
    pub(crate) fn model_context_window(&self) -> Option<i64> {
        ...
        self.effective_raw_context_window()
            .map(|context_window| {
