...
async fn run_compact_task_inner_impl(
    ...
    let user_message_retention_budget: usize = if turn_context.config.simple_context {
        let context_window = turn_context
            .effective_raw_context_window()
            .unwrap_or(0);
        (context_window.saturating_sub(2048).max(0) as f64 * 0.3) as usize
    } else {
