...
pub fn build_models_manager(
    ...
    provider.models_manager(
        ...
        config.system_prompt.text.clone(),
    )
}
...
impl ThreadManager {
    ...
    /// Like `with_models_provider_for_tests`, but the models manager serves
    /// the given catalog instead of the bundled millie catalog. Tier and
    /// migration tests need upstream models the millie catalog does not carry.
    pub(crate) fn with_models_provider_and_catalog_for_tests(
        ...
        let state = Arc::get_mut(&mut manager.state)
            .expect("fresh test ThreadManager state is uniquely owned");
        state.models_manager = Arc::new(
            ...
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(
                        ...
                        codex_models_manager::system_prompt::resolve_system_prompt_for_tests(
                            &codex_home,
                        )
                        .text,
                    ),
