...
/// Call-id prefix of harness notes (continuation prompts, loop-guard notes)
/// recorded as tool results with no preceding call; see
/// `session::turn::harness_note_item`. They are kept deliberately.
pub(crate) const HARNESS_NOTE_CALL_PREFIX: &str = "harness_";
...
pub(crate) fn remove_orphan_outputs(items: &mut Vec<ResponseItem>) {
    ...
    items.retain(|item| match item {
        ResponseItem::FunctionCallOutput { call_id, .. } if call_id.starts_with(HARNESS_NOTE_CALL_PREFIX) => true,
