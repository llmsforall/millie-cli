...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        // Barrier: if a background prompt-cache warm-up is in flight on the
        // single-slot local server, wait for it to finish before starting the
        // turn, then reuse its prefilled cache. Acquire-and-release only; the
        // turn does not hold this across its lifetime (a new warm-up is only
        // spawned at turn end, so nothing races the turn once past here).
        self.prewarm_cancel.lock().await.cancel();
        drop(self.prewarm_lock.clone().lock_owned().await);
        ...
        let handle = tokio::spawn(
            async move {
                ...
                // Take the prewarm lock the instant generation ends -- before
                // flush_rollout or any other await -- so a next turn (even an
                // instant one) is already blocked at the start_task barrier and
                // cannot race the mandatory warm-up. Held across teardown and
                // dropped inside on_task_finished after the warm-up runs.
                let prewarm_guard = sess.acquire_prewarm_guard_if_local().await;
                ...
                if !task_cancellation_token.is_cancelled() {
                    ...
                    sess.on_task_finished(Arc::clone(&ctx_for_finish), last_agent_message, prewarm_guard)
                        .await;
                } else {
                    // Cancelled turns skip teardown; release the prewarm lock so
                    // the replacing turn's barrier is not held behind it.
                    drop(prewarm_guard);
                }
    ...
    pub async fn on_task_finished(
        ...
        // Prewarm lock, taken by the caller the instant generation ended (before
        // any await), so an instant next turn is already blocked at the
        // start_task barrier. Held across teardown and dropped after the
        // mandatory warm-up runs below. `None` for remote providers.
        prewarm_guard: Option<tokio::sync::OwnedMutexGuard<()>>,
    ) {
        turn_context
            .turn_metadata_state
        ...
        // Mandatory prompt-cache warm-up. Run it inline here, the instant the
        // turn finishes, holding `prewarm_guard` (taken at the top of teardown)
        // so it always runs and always completes before the session accepts the
        // next turn. This leaves the local server's cache in the exact
        // think-stripped form the next turn's prefix matches directly, so the
        // next turn never has to roll back to a checkpoint for the shrink case.
        // No-op for remote providers (guard is None, request builder returns
        // None). Kept before the `cleared_active_turn` gate so an interleaved
        // next turn cannot cause it to be skipped.
        if prewarm_guard.is_some() {
            self.prewarm_prompt_cache_blocking(turn_context.as_ref())
                .await;
        }
        drop(prewarm_guard);
