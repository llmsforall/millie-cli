...
fn thread_manager() -> ThreadManager {
    // tier/fork tests reference upstream catalog models (gpt-5.4 etc.) that
    // the bundled millie catalog does not carry
...
/// Role/tier tests exercise upstream catalog semantics; the llamacpp model
/// force (llamacpp sessions always resolve to a millie model) would rewrite
/// their gpt-* fixtures, so run them under the openai provider instead.
...
/// The bundled millie catalog has no gpt-* models or service tiers; these
/// tier tests exercise upstream catalog machinery, so give the session the
/// upstream models.json catalog explicitly.
fn use_upstream_test_catalog(session: &mut crate::session::session::Session) {
    let catalog = codex_models_manager::bundled_models_response()
        .expect("bundled models.json should parse");
    session.services.models_manager = std::sync::Arc::new(
        codex_models_manager::manager::StaticModelsManager::new(None, catalog),
    );
}
...
async fn spawn_agent_service_tier_override_validates_the_effective_child_model() {
    ...
    {
        let (mut session, mut turn) = make_session_and_context().await;
        use_upstream_test_catalog(&mut session);
