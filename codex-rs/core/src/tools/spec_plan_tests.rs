...
fn set_edit_dialect(turn: &mut TurnContext, edit_dialect: EditDialect) {
    let mut config = (*turn.config).clone();
    config.edit_dialect = edit_dialect;
    turn.config = Arc::new(config);
}
...
#[tokio::test]
async fn default_edit_dialect_registers_str_replace_tools() {
    let plan = probe(|turn| {
        turn.model_info.apply_patch_tool_type = Some(ApplyPatchToolType::Freeform);
    })
    .await;

    plan.assert_visible_contains(&["create_file", "str_replace", "delete_file"]);
    plan.assert_visible_lacks(&["apply_patch"]);
    plan.assert_registered_contains(&["create_file", "str_replace", "delete_file"]);
    plan.assert_registered_lacks(&["apply_patch"]);
}

#[tokio::test]
async fn apply_patch_edit_dialect_registers_apply_patch_tool_only() {
    let plan = probe(|turn| {
        set_edit_dialect(turn, EditDialect::ApplyPatch);
        turn.model_info.apply_patch_tool_type = Some(ApplyPatchToolType::Freeform);
    })
    .await;

    plan.assert_visible_contains(&["apply_patch"]);
    plan.assert_visible_lacks(&["create_file", "str_replace", "delete_file"]);
    plan.assert_registered_contains(&["apply_patch"]);
    plan.assert_registered_lacks(&["create_file", "str_replace", "delete_file"]);
}
