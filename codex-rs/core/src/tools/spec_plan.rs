...
use crate::tools::handlers::CreateFileHandler;
...
use crate::tools::handlers::StrReplaceHandler;
...
fn add_tool_sources(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    if context.turn_context.config.extension_tools_only {
        // Extension-only surface: no shell, apply_patch, MCP, or multi-agent
        // tools -- only whatever extensions contribute.
...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    ...
    if environment_mode.has_environment() {
        match turn_context.config.edit_dialect {
            EditDialect::StrReplace => {
                planned_tools.add(CreateFileHandler);
                planned_tools.add(StrReplaceHandler);
                planned_tools.add(DeleteFileHandler);
            }
            EditDialect::ApplyPatch => {
