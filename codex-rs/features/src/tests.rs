        ...
        if spec.default_enabled {
            assert!(
                matches!(spec.stage, Stage::Stable | Stage::Removed)
                    ...
                    // Millie deliberately keeps the Landlock + seccomp sandbox
                    // as the default (deprecated upstream, but our
                    // dependency-free Linux sandbox; bwrap is not bundled).
                    || spec.id == Feature::UseLegacyLandlock,
...
#[test]
fn use_legacy_landlock_is_deprecated_but_enabled_by_default() {
    // Millie keeps the Landlock + seccomp sandbox as the default (no bundled
    // bubblewrap); upstream deprecated it but we depend on it.
    assert_eq!(Feature::UseLegacyLandlock.stage(), Stage::Deprecated);
    assert_eq!(Feature::UseLegacyLandlock.default_enabled(), true);
}
