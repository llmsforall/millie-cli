...
pub struct OpenAiModelsManager {
    ...
    /// The runtime model catalog (see `crate::catalog`) with the served
    /// system prompt installed into every entry, resolved once at startup.
...
impl OpenAiModelsManager {
    ...
    pub fn new(
        ...
        let catalog_models = crate::catalog::millie_models(&system_prompt, &codex_home)
            .unwrap_or_else(|err| {
                tracing::error!("model catalog unavailable: {err}");
                Vec::new()
            });
        let remote_models = catalog_models.clone();
        Self {
            catalog_models,
...
impl OpenAiModelsManager {
    ...
    async fn apply_remote_models(&self, models: Vec<ModelInfo>) {
        ...
        let mut existing_models = self.catalog_models.clone();
        for model in models {
