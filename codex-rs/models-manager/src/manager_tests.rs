...
/// Test-local stand-in for the removed loader: the catalog read from the checkout.
fn load_remote_models_from_file(system_prompt: &str) -> Result<Vec<ModelInfo>, std::io::Error> {
    Ok(crate::bundled_millie_models(system_prompt))
}

fn test_system_prompt() -> String {
    crate::system_prompt::resolve_system_prompt_for_tests(std::path::Path::new(
        "/nonexistent-millie-home",
    ))
    .text
}
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let bundled_slug = load_remote_models_from_file(&test_system_prompt())
...
async fn refresh_available_models_preserves_bundled_catalog_for_empty_chatgpt_remote() {
    ...
    let manager = openai_manager_for_tests(codex_home.path().to_path_buf(), endpoint);
    let expected = load_remote_models_from_file(&test_system_prompt()).expect("bundled models should parse");
...
async fn refresh_available_models_merges_hidden_only_chatgpt_remote_with_bundled_catalog() {
    ...
    let manager = openai_manager_for_tests(codex_home.path().to_path_buf(), endpoint);
    let mut expected = load_remote_models_from_file(&test_system_prompt()).expect("bundled models should parse");
...
#[test]
fn bundled_catalog_is_millie_only() {
    // The regression this pins: identity/branding fixes used to edit
    // upstream's models.json in place, and every upstream sync brought
    // GPT-5.5/Codex/OpenAI text back into the UI. The PRODUCT catalog is
    // bundled_millie_models(), loaded instead of models.json; if this test
    // fails, someone broke that — do NOT fix it by editing models.json.
    let prompt = crate::system_prompt::resolve_system_prompt_for_tests(std::path::Path::new("/nonexistent-millie-home"));
    let models = crate::bundled_millie_models(&prompt.text);
    let slugs: Vec<&str> = models.iter().map(|m| m.slug.as_str()).collect();
    assert_eq!(slugs, vec!["millie-35B-A3B-7GB", "millie-35B-A3B-9GB", "millie-35B-A3B-11GB"]);
    for m in &models {
        let hay = format!(
            "{} {} {:?} {:?}",
            m.display_name,
        ...
        for word in ["GPT", "OpenAI", "Codex", "gpt-5"] {
            assert!(!hay.contains(word), "forbidden brand {word:?} in {hay}");
        }
        // The served prompt is the runtime prompt file with the default
        // personality rendered in -- never a compiled-in text.
        let default_personality = m
            .model_messages
            .as_ref()
            .and_then(|mm| mm.get_personality_message(None))
            .unwrap_or_default();
        assert_eq!(
            m.base_instructions,
            prompt
                .text
                .replace(crate::system_prompt::PERSONALITY_PLACEHOLDER, &default_personality)
        );
        assert!(m.base_instructions.contains("## File editing: create_file and str_replace"));
        assert!(!m.base_instructions.contains("apply_patch"));
        assert!(!m.base_instructions.contains("{{ personality }}"));
        assert_eq!(
            m.model_messages.as_ref().and_then(|mm| mm.instructions_template.clone()),
