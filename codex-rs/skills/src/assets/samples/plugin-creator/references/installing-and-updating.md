...
- the user wants Millie to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single Millie cachebuster suffix:
...
outside Millie depends on a specific token:
...
6. Prompt the user to use a new thread to try the updated plugin, so that Millie picks up new skills
   and tools.
...
Replace the existing Millie cachebuster instead of appending another one. Do not keep incrementing
...
- Do not tell the user to run `codex plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Millie.
