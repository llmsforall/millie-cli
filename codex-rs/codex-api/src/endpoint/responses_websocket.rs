...
use codex_client::llm_websocket_response_recorder;
use codex_client::log_llm_traffic_error;
use codex_client::log_llm_websocket_connect;
use codex_client::log_llm_websocket_connected;
use codex_client::log_llm_websocket_outgoing;
use codex_client::maybe_build_rustls_client_config_with_custom_ca;
use codex_client::next_llm_traffic_exchange_id;
...
async fn connect_websocket(
    ...
    let exchange_id = next_llm_traffic_exchange_id();
    log_llm_websocket_connect(exchange_id, url.as_str(), &headers);
    ...
    let (stream, response) = match response {
        ...
        Err(err) => {
            ...
            log_llm_traffic_error(exchange_id, err.to_string());
...
async fn run_websocket_response_stream(
    ...
    let exchange_id = send_websocket_request(
        ws_stream,
        request_body,
        idle_timeout,
        telemetry.as_ref(),
    ...
    .await?;
    let mut response_recorder = llm_websocket_response_recorder(exchange_id);
    ...
    loop {
        ...
        let message = match response {
            ...
            Ok(Some(Err(err))) => {
                log_llm_traffic_error(exchange_id, err.to_string());
                response_recorder.abort(err.to_string());
                return Err(ApiError::Stream(err.to_string()));
            }
            Ok(None) => {
                let message = "stream closed before response.completed";
                log_llm_traffic_error(exchange_id, message);
                response_recorder.abort(message);
                return Err(ApiError::Stream(message.into()));
            }
            Err(err) => {
                log_llm_traffic_error(exchange_id, err.to_string());
                response_recorder.abort(err.to_string());
        ...
        match message {
            Message::Text(text) => {
                ...
                {
                    response_recorder.abort(format!("{error:?}"));
                    return Err(error);
                }
            ...
            Message::Binary(_) => {
                log_llm_traffic_error(exchange_id, "unexpected binary websocket event");
                response_recorder.abort("unexpected binary websocket event");
                return Err(ApiError::Stream("unexpected binary websocket event".into()));
            }
            Message::Close(_) => {
                let message = "websocket closed by server before response.completed";
                log_llm_traffic_error(exchange_id, message);
                response_recorder.abort(message);
                return Err(ApiError::Stream(message.into()));
            }
...
async fn send_websocket_request(
    ...
    let exchange_id = next_llm_traffic_exchange_id();
    log_llm_websocket_outgoing(exchange_id, &request_text);
