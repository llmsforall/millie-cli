...
///
/// 1.  Load `.env` values from `~/.millie/.env` before creating any threads.
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "MILLIE_";

/// Load env vars from ~/.millie/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `MILLIE_`.
fn load_dotenv() {
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    // Use a MILLIE_HOME-scoped temp root to avoid cluttering the top-level directory.
