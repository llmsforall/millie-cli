...
//!   2. User-defined entries inside `~/.millie/config.toml` under the `model_providers`
...
pub const DEFAULT_LLAMACPP_PORT: u16 = 8090;

pub const LMSTUDIO_OSS_PROVIDER_ID: &str = "lmstudio";
pub const OLLAMA_OSS_PROVIDER_ID: &str = "ollama";
pub const LLAMACPP_OSS_PROVIDER_ID: &str = "llamacpp";
pub const DEFAULT_VLLM_PORT: u16 = 8300;
/// A vLLM server serving a Millie model (attach-only; the server is started separately).
...
pub fn built_in_model_providers(
    ...
    [
        ...
        (
            ...
            ModelProviderInfo {
                // Distinct from the shared "gpt-oss" name Ollama/LM Studio
                // get from `create_oss_provider`: the raw-completions
                // transport dispatch in codex-core's `stream()` identifies
                // this provider by name, so it needs to actually be unique.
                name: LLAMACPP_OSS_PROVIDER_ID.to_string(),
                ..create_oss_provider_with_base_url(
                    &format!("http://127.0.0.1:{}/v1", llamacpp_port()),
                    WireApi::Responses,
                )
            },
        ),
        (
            ...
            ModelProviderInfo {
                // Same chat transport as llamacpp with the vLLM request shape;
                // dispatched by name in codex-core's `stream()`.
                name: VLLM_OSS_PROVIDER_ID.to_string(),
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    ...
    let default_codex_oss_base_url = format!(
        ...
        codex_oss_port = std::env::var("MILLIE_OSS_PORT")
            .ok()
    ...
    let codex_oss_base_url = std::env::var("MILLIE_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
...
/// Managed llama-server port, shared by launch, identity checks and inference.
pub fn llamacpp_port() -> u16 {
    std::env::var("MILLIE_LLAMACPP_PORT")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| {
            std::env::var("MILLIE_OSS_PORT")
                .ok()
                .filter(|v| !v.trim().is_empty())
        })
        .and_then(|v| v.trim().parse().ok())
        .unwrap_or(DEFAULT_LLAMACPP_PORT)
}

/// Normalize either a server root or an OpenAI /v1 base URL to its root.
pub fn local_server_root(base_url: &str) -> &str {
...
mod local_endpoint_tests {
    #[test]
    fn server_roots_accept_api_suffix_and_trailing_slashes() {
        for url in [
            "http://host:8300",
            "http://host:8300/",
            "http://host:8300/v1",
            ...
            assert_eq!(super::local_server_root(url), "http://host:8300");
        }
    }
}
