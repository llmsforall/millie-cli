...
mod tests {
    ...
    async fn cloud_requirements_eligible_auth_allows_agent_identity_business_plan() {
        ...
        let original_authapi_base_url = std::env::var_os("MILLIE_AGENT_IDENTITY_AUTHAPI_BASE_URL");
        unsafe {
            std::env::set_var("MILLIE_AGENT_IDENTITY_AUTHAPI_BASE_URL", &authapi_base_url);
        }
        let _authapi_guard = EnvVarGuard {
            key: "MILLIE_AGENT_IDENTITY_AUTHAPI_BASE_URL",
