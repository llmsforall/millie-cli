...
async fn sandbox_blocks_first_time_dot_codex_creation() {
    ...
    let dot_codex = repo_root.join(".millie");
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .millie && echo 'sandbox_mode = \"danger-full-access\"' > .millie/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .millie");

    let status = child.wait().await.expect("should wait for .millie command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .millie creation: {status:?}"
    );
