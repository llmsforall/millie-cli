...
pub use codex_utils_oss::model_updates;
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let codex_home = match find_codex_home() {
        ...
        Err(err) => {
            eprintln!("Error finding the Millie home directory: {err}");
            std::process::exit(1);
        }
    };
    ...
    if keep_model_server {
        // Startup-only export, before the provider supervisor is spawned.
        unsafe {
            std::env::set_var("MILLIE_LLAMACPP_KEEP_ALIVE", "1");
        }
    }
    let model_provider = if oss_provider.is_some() {
        ...
        config_toml.model_provider.as_deref(),
        Some(LLAMACPP_OSS_PROVIDER_ID) | Some(VLLM_OSS_PROVIDER_ID)
    ) {
        // `model_provider = "llamacpp"` (or "vllm") in config.toml selects the
        // provider for the request path (see the transport dispatch on
        // provider name), so the launch side -- env exports below and
        // `ensure_oss_provider_ready` -- must run for it too. Without this,
        // config-only selection sends completions to the default port while
        // no server is ensured on it.
        config_toml.model_provider.clone()
    } else if config_toml.model_provider.is_none() {
        // No provider configured: codex-core falls back to the local llamacpp
        // provider, so the launch side (env exports, server bootstrap) must
        // follow the same default or `--llama-*` flags silently do nothing.
        Some(LLAMACPP_OSS_PROVIDER_ID.to_string())
    } else {
        None // a configured non-local provider
    };

    // For the local llama.cpp provider, resolve model path / GPU selection /
    // port / context size (CLI flag overrides config.toml's `[llamacpp]`
    // section, which overrides built-in defaults) and export them as
    // `CODEX_LLAMACPP_*` env vars, following the same pattern
    // `create_oss_provider` already uses for `CODEX_OSS_PORT`/
    // `CODEX_OSS_BASE_URL`. `codex-llamacpp::ensure_oss_ready` reads these
    // when it's actually invoked below, so this is a no-op unless the
    // `llamacpp` provider is selected.
    if model_provider.as_deref() == Some(VLLM_OSS_PROVIDER_ID) {
        // vLLM: attach-only. The server URL and served model name come from
        // `[vllm]`; the generation settings below (`[llamacpp]`) apply too.
        let vllm_toml = config_toml.vllm.clone().unwrap_or_default();
        if let Some(url) = vllm_toml.base_url {
            unsafe {
                std::env::set_var("MILLIE_OSS_BASE_URL", url);
            }
        }
        if let Some(name) = vllm_toml.served_model {
            unsafe {
                std::env::set_var("MILLIE_VLLM_MODEL", name);
            }
        }
    }
    if matches!(
        ...
        let llamacpp_toml = config_toml.llamacpp.clone().unwrap_or_default();

        // Persisted prompt caches live under the resolved Millie home; the
        // server loads a matching one at startup so the system prompt need
        // not be re-prefilled.
        // SAFETY: single-threaded at this point in startup.
        unsafe {
            std::env::set_var(
                "MILLIE_LLAMACPP_CACHE_BASE",
                codex_home.join("kvcache").to_path_buf(),
            );
        }
        // The launched server's log goes under the resolved Millie home.
        // SAFETY: single-threaded at this point in startup.
        unsafe {
            std::env::set_var(
                "MILLIE_LLAMACPP_SERVER_LOG",
                codex_home.join("llama-server.log").to_path_buf(),
            );
        }
        ...
        if download_approved {
            // SAFETY: single-threaded at this point in startup.
            unsafe {
                std::env::set_var("MILLIE_DOWNLOAD_APPROVED", "1");
            }
        }
        if let Some(path) = llama_model_path.or(llamacpp_toml.model_path.map(|p| p.to_path_buf())) {
            // SAFETY: single-threaded at this point in startup, before any
            // work that reads these env vars is spawned.
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_MODEL_PATH", path);
            }
        }
        if let Some(model_hf) = llamacpp_toml.model_hf.clone() {
            // SAFETY: single-threaded startup, as above.
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_MODEL_HF", model_hf);
            }
        }
        if let Some(gpu) = llama_gpu.or(llamacpp_toml.gpu) {
            // An empty list means CPU-only; the env var can't carry an empty
            // value distinctly from "unset", so it's exported as "none".
            let joined = if gpu.is_empty() {
                "none".to_string()
            } else {
                gpu.iter().map(u32::to_string).collect::<Vec<_>>().join(",")
            };
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_GPU", joined);
            }
        }
        if let Some(port) = llama_port.or(llamacpp_toml.port) {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_PORT", port.to_string());
                // The request-side base_url is built by `create_oss_provider`
                // from `CODEX_OSS_PORT`; without this the server launches on
                // the configured port while completions go to the default.
                std::env::set_var("MILLIE_OSS_PORT", port.to_string());
            }
        }
        if let Some(parallel) = llamacpp_toml.parallel {
            // SAFETY: single-threaded startup, as above.
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_PARALLEL", parallel.to_string());
            }
        }
        if let Some(mmproj_path) = llamacpp_toml.mmproj_path.clone() {
            // SAFETY: single-threaded startup, as above.
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_MMPROJ_PATH", mmproj_path.to_path_buf());
            }
        }
        if let Some(ctx_size) = llamacpp_toml.ctx_size {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_CTX_SIZE", ctx_size.to_string());
            }
        }
        let vision_override = if vision {
            Some(true)
        } else if no_vision {
            Some(false)
        } else {
            llamacpp_toml.vision
        };
        if let Some(vision) = vision_override {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_VISION", if vision { "1" } else { "0" });
            }
        }
        if let Some(kv_cache) = llamacpp_toml.kv_cache.clone() {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_KV_CACHE", kv_cache);
            }
        }
        if let Some(mmap) = llamacpp_toml.mmap {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_MMAP", if mmap { "1" } else { "0" });
            }
        }
        if let Some(n_cpu_moe) = llamacpp_toml.n_cpu_moe {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_N_CPU_MOE", n_cpu_moe.to_string());
            }
        }
        if let Some(profile) = serving_profile
            .clone()
            .or_else(|| llamacpp_toml.profile.clone())
        {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_PROFILE", profile);
            }
        }
        if let Some(server_bin) = llamacpp_toml.server_bin {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_SERVER_BIN", server_bin.to_path_buf());
            }
        }
        if let Some(temperature) = llamacpp_toml.temperature {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_TEMPERATURE", temperature.to_string());
            }
        }
        if let Some(min_p) = llamacpp_toml.min_p {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_MIN_P", min_p.to_string());
            }
        }
        if let Some(top_k) = llamacpp_toml.top_k {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_TOP_K", top_k.to_string());
            }
        }
        if let Some(top_p) = llamacpp_toml.top_p {
            unsafe {
                std::env::set_var("MILLIE_LLAMACPP_TOP_P", top_p.to_string());
            }
        }
        if let Some(max_output_tokens) = llamacpp_toml.max_output_tokens {
            unsafe {
                std::env::set_var(
        ...
        if let Some(thinking_budget) = llamacpp_toml.thinking_budget {
            unsafe {
                std::env::set_var(
        ...
        let pathology_env: [(&str, Option<String>); 7] = [
            (
                "MILLIE_LLAMACPP_REPEAT_STOP",
                llamacpp_toml.repeat_stop.map(|v| v.to_string()),
            ),
            (
                "MILLIE_LLAMACPP_NGRAM_PENALTY_SCALE",
                llamacpp_toml.ngram_penalty_scale.map(|v| v.to_string()),
            ),
            (
                "MILLIE_LLAMACPP_NGRAM_PENALTY_STEP",
                llamacpp_toml.ngram_penalty_step.map(|v| v.to_string()),
            ),
            (
                "MILLIE_LLAMACPP_NGRAM_PENALTY_START_N",
                llamacpp_toml.ngram_penalty_start_n.map(|v| v.to_string()),
            ),
            (
                "MILLIE_LLAMACPP_NGRAM_PENALTY_MAX",
                llamacpp_toml.ngram_penalty_max.map(|v| v.to_string()),
            ),
            (
                "MILLIE_LLAMACPP_BACKTRACK_ON_REPETITION",
                llamacpp_toml.backtrack_on_repetition.map(|v| v.to_string()),
            ),
            (
                "MILLIE_LLAMACPP_RESAMPLES",
                llamacpp_toml.resamples.map(|v| v.to_string()),
            ),
        ];
        for (key, value) in pathology_env {
        ...
        // Pre-bootstrap default for the agent-side window. The real value is
        // resolved at launch: the profile layer takes min(this window, the
        // machine-fit cap) for the server's --ctx-size, and the launcher
        // exports the confirmed serving size (MILLIE_LLAMACPP_RESOLVED_CTX),
        // which TurnContext min()s in per call -- so this constant only
        // covers displays before the first server contact.
        if model_context_window.is_none() && config_toml.model_context_window.is_none() {
            model_context_window = Some(codex_llamacpp::DEFAULT_CTX_SIZE as i64);
        }
    }

    let model = if model_provider.as_deref()
        == Some(codex_model_provider_info::LLAMACPP_OSS_PROVIDER_ID)
        || (model_provider.as_deref() == Some(codex_model_provider_info::VLLM_OSS_PROVIDER_ID)
            && model_cli_arg
                .as_deref()
                .is_some_and(|model| model != "select"))
    {
        Some(codex_utils_oss::select_startup_model(
            &codex_home,
            loader_overrides
                .user_config_path
                .as_deref()
                .unwrap_or(&codex_home.join("config.toml")),
            model_cli_arg.as_deref(),
            config_toml.model.as_deref(),
        )?)
    } else {
        model_cli_arg
            .as_deref()
            .map(str::to_owned)
            .or_else(|| config_toml.model.clone())
    };
    ...
    let overrides = ConfigOverrides {
        ...
        simple_context: simple_context.then_some(true),
        extension_tools_only: None,
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    info!("Millie initialized with event: {session_configured:?}");
...
fn thread_start_params_from_config(config: &Config) -> ThreadStartParams {
    ...
    ThreadStartParams {
        ...
        permissions,
        config: None,
        extension_tools_only: Some(config.extension_tools_only),
...
fn thread_resume_params_from_config(config: &Config, thread_id: String) -> ThreadResumeParams {
    ...
    ThreadResumeParams {
        ...
        permissions,
        config: None,
        // Each `codex exec` invocation is a separate process; `resume` cold-reloads
        // the session from its persisted rollout rather than continuing an
        // in-memory server. `extension_tools_only` is a runtime-only knob with no
        // config.toml persistence (same category as `simple_context`), so it only
        // reflects the truth if the embedding caller re-specifies it on resume.
        extension_tools_only: Some(config.extension_tools_only),
...
/// Dispatch the model-server supervisor before terminal/config initialization.
pub use codex_llamacpp::SUPERVISOR_ARG as MODEL_SUPERVISOR_ARG;

pub fn dispatch_model_supervisor() -> Option<std::io::Result<()>> {
    if std::env::args().nth(1).as_deref() != Some(codex_llamacpp::SUPERVISOR_ARG) {
        return None;
    }
    Some(
        tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .and_then(|runtime| runtime.block_on(codex_llamacpp::run_supervisor())),
    )
}
