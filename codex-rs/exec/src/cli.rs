...
#[command(
    version,
    override_usage = "millie-cli exec [OPTIONS] [PROMPT]\n       millie-cli exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Millie outside a Git repository.
    ...
    /// Do not load `$MILLIE_HOME/config.toml`; auth still uses `MILLIE_HOME`.
    ...
    /// Approve downloading the model if it is not cached yet. Without this
    /// (or a `model` set in config.toml), a non-interactive run refuses to
    /// start a multi-gigabyte download.
    #[arg(long = "download", global = true, default_value_t = false)]
    pub download: bool,

    /// Path to the .gguf model file to serve via the local `llamacpp` OSS
    /// provider. Overrides `llamacpp.model_path` in config.toml.
    #[arg(long = "llama-model-path", value_name = "FILE", global = true)]
    pub llama_model_path: Option<PathBuf>,

    /// GPU indices to expose to `llama-server` via `CUDA_VISIBLE_DEVICES`,
    /// e.g. `--llama-gpu 0` or `--llama-gpu 0,1`. Defaults to a single GPU
    /// (index 0). Pass an empty value for CPU-only. Overrides `llamacpp.gpu`
    /// in config.toml.
    #[arg(
        long = "llama-gpu",
        value_name = "INDICES",
    ...
    /// Port for `llama-server` to listen on (and where Millie looks for an
    /// already-running instance before launching a new one). Overrides
    /// `llamacpp.port` in config.toml.
    #[arg(long = "llama-port", value_name = "PORT", global = true)]
    ...
    #[arg(long = "serving-profile", value_name = "NAME", global = true)]
    ...
    #[arg(long = "vision", global = true, default_value_t = false, conflicts_with = "no_vision")]
    pub vision: bool,

    /// Force text-only serving (no image input), saving the vision tower's
    /// memory.
    #[arg(long = "no-vision", global = true, default_value_t = false)]
    pub no_vision: bool,
...
fn mark_exec_global_args(cmd: clap::Command) -> clap::Command {
    cmd.mut_arg("model", |arg| arg.global(true))
        ...
        .mut_arg("simple_context", |arg| arg.global(true))
        .mut_arg("model_context_window", |arg| arg.global(true))
}
