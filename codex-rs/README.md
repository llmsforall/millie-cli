...
`$MILLIE_HOME/NAME.config.toml` onto the base user config for the sandboxed
command.
...
In `workspace-write`, Codex also includes `~/.millie/memories` in its writable roots so memory maintenance does not require an extra approval.
