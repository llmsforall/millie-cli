...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.millie/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
...
impl InstallContext {
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("MILLIE_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("MILLIE_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
