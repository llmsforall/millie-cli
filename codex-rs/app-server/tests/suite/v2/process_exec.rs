...
async fn process_spawn_returns_before_exit_and_emits_exit_notification() -> Result<()> {
    ...
    let command = if cfg!(windows) {
        vec![
            ...
            "-Command".to_string(),
            concat!(
                "[IO.File]::WriteAllText($env:MILLIE_PROCESS_EXEC_PROBE_FILE, 'process'); ",
                "while (!(Test-Path -LiteralPath $env:MILLIE_PROCESS_EXEC_RELEASE_FILE)) { ",
        ...
        vec![
            ...
            "-c".to_string(),
            concat!(
                "printf process > \"$MILLIE_PROCESS_EXEC_PROBE_FILE\"; ",
                "while [ ! -e \"$MILLIE_PROCESS_EXEC_RELEASE_FILE\" ]; do sleep 0.05; done; ",
                "printf process-out; ",
    ...
    let env = HashMap::from([
        (
            "MILLIE_PROCESS_EXEC_PROBE_FILE".to_string(),
            Some(probe_file.display().to_string()),
        ),
        (
            "MILLIE_PROCESS_EXEC_RELEASE_FILE".to_string(),
