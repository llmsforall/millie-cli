...
/// Shared app-server entry point for loading effective Millie configuration.
#[derive(Clone)]
pub(crate) struct ConfigManager {
    ...
    /// Launch-time --simple-context / --model-context-window flags. These are session-wide and
    /// never change at runtime, unlike `cli_overrides`, so they're plain fields rather than
    /// `RwLock`-wrapped. Persisted here (rather than only in the initial bootstrap `Config`
    /// build) because every subsequent config load in this process goes through
    /// `load_with_cli_overrides` with a fresh, per-call `ConfigOverrides` that has no way to know
    /// about launch-time flags otherwise.
    simple_context: bool,
    model_context_window_override: Option<i64>,
}
...
impl ConfigManager {
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        // Carry forward launch-time --simple-context/--model-context-window unless this specific
        // call already set them explicitly. No caller currently sets these, so in practice this
        // is what makes them apply to every load; when simple_context was never passed at launch,
        // `self.simple_context` is `false` and `then_some(true)` is `None`, so this is a no-op
        // and behavior is unchanged from before this field existed.
        if typesafe_overrides.simple_context.is_none() {
            typesafe_overrides.simple_context = self.simple_context.then_some(true);
        }
        if typesafe_overrides.model_context_window.is_none() {
            typesafe_overrides.model_context_window = self.model_context_window_override;
        }
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            /*model_context_window_override*/ None,
        )
    }
