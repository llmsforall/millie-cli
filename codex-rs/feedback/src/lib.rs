...
/// Sentry DSN for feedback uploads. Not set in this build: feedback upload is
/// unavailable unless a DSN is supplied at compile time.
const SENTRY_DSN: Option<&str> = option_env!("MILLIE_FEEDBACK_SENTRY_DSN");
...
impl FeedbackSnapshot {
    ...
    pub fn upload_feedback(&self, options: FeedbackUploadOptions<'_>) -> Result<()> {
        ...
        let Some(dsn) = SENTRY_DSN else {
            return Err(anyhow!("feedback upload is not configured in this build"));
        };
        // Build Sentry client
