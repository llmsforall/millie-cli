...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=MILLIE_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=MILLIE_SKIP_BWRAP_BUILD");
    ...
    if target_os != "linux" || env::var_os("MILLIE_SKIP_BWRAP_BUILD").is_some() {
        return;
    }
...
///
/// Priority:
/// 1. `MILLIE_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
...
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("MILLIE_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "MILLIE_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
